...
/**
 *  Crea el vector de sufijos que se usara en BWT
 */
...
/**
 *  Dada una secuencia de indices, calcula su cadena de texto correspondiente invirtiendo el algoritmo MTF
 */
...
/**
 * Dada una cadena input que es una transformada Burrows-Wheeler, devuelve
 * la cadena de la que se ha obtenido dicha transformada
 */
...
int main(int argc, char const *argv[])
{
    ...
    {   
        ...
        // Obtener tiempo de compresión
        clock_gettime(CLOCK_REALTIME, &finish);
        double elapsed = (finish.tv_sec - start.tv_sec);
        elapsed += (finish.tv_nsec - start.tv_nsec) / 1000000000.0;
        cout << "Tiempo transcurrido para comprimir: " << elapsed << " segundos." << std::endl;
    }
    else if (string(argv[1]) == "-d") //DESCOMPRIMIR
    {
        //Medir tiempo descomprimir
        struct timespec start, finish;
        clock_gettime(CLOCK_REALTIME, &start);
        ...
        // Obtener tiempo de descomprimir
        clock_gettime(CLOCK_REALTIME, &finish);
        double elapsed = (finish.tv_sec - start.tv_sec);
        elapsed += (finish.tv_nsec - start.tv_nsec) / 1000000000.0;
        cout << "Tiempo transcurrido para descomprimir: " << elapsed << " segundos." << std::endl;
    }
    else
    {
